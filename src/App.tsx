...
import { ArrowLeftRight, ArrowUpRight, Check, ChevronDown, ChevronRight, CircleHelp, Focus, Hand, Layers2, Link2, LoaderCircle, Maximize, Minus, Move, PersonStanding, Plus, RotateCcw, Rotate3D, Search, SlidersHorizontal, Sparkles, Undo2, Upload, X } from 'lucide-react';
...
export default function App() {
  ...
  return <div className="app-shell">
    <header className="app-header">
      ...
      <div className="header-actions"><button className="about-button" onClick={()=>aboutRef.current?.showModal()}><CircleHelp size={16}/><span>About the model</span></button></div>
    </header>
    ...
    <dialog ref={aboutRef} className="about-dialog" onClick={e=>{if(e.target===aboutRef.current)aboutRef.current?.close();}}>
      ...
      <section><h3>Your shape stays here</h3><p>Adjustments are saved automatically in this browser. “Import shape” can restore files from earlier versions. No account, body measurements or photographs are requested. Shape adjustments stay on your device. The site loads anatomy assets and Google Fonts over the network; no analytics or tracking scripts are added by the app.</p></section>
